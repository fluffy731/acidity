...
export default async function Staffing() {
  ...
  return <>
    <PageHeading title="Roster">Who is on, what it costs at base rate, and whether every gig night has the people it needs.</PageHeading>
    <div className="kpis">
      <Kpi label="Rostered hours" value={`${summary.hours} h`} hint={`${summary.shifts} shifts`} />
      <Kpi label="Rostered cost" value={currency(summary.cost)} hint="Base hourly rate only - no penalty rates" />
      <Kpi label="Worked so far" value={currency(worked.cost)} hint={`${worked.hours} h marked worked`} />
