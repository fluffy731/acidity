...
/** Today / Programme / Stock / Roster / Money - the five things a bar owner checks. Staff
 * see the roster and today's programme only. */
export function AppShell({ children, live = false, user }: { children: ReactNode; live?: boolean; user?: { name: string; role: string } | null }) {
  ...
  return <MobileNavProvider><header className="app-header"><Link href="/dashboard" className="brand" aria-label="Acidic Today"><AcidicLogo /></Link><MobileNavButton /><nav aria-label="Main navigation"><Link href="/dashboard">Today</Link><Link href="/programme">Programme</Link><Link href="/staffing">Roster</Link>{manager ? <Link href="/accounting">Money</Link> : null}</nav><div className="header-right"><span>{live ? `${user?.name ?? "Signed in"} · ${user?.role ?? ""}` : "Preview"}</span>{live ? <form action={async () => { "use server"; await signOut({ redirectTo: "/login" }); }}><button className="secondary">Sign out</button></form> : <Link href="/login">Exit preview</Link>}</div></header>
